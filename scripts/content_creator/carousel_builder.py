...
{{
  ...
  "proof_needed": "name the SPECIFIC data point and its actual publisher — not just the organization name. Source-topic rules: NAHB = construction cost category shares only (NOT drainage, NOT specialty repair costs, NOT exact concrete-vs-wood repair ranges). ACI/TMS = masonry/concrete structural standards only (NOT homeowner costs, NOT maintenance ranges). UF IFAS = termites and wood-destroying organism risk. FBC/IRC = code minimums only, never cost estimates. Angi/HomeAdvisor = BANNED as primary proof for OPC numeric claims; they are consumer quote guides, not audit-grade evidence. ACI 314.1R is BANNED — do not cite it. If the number you want is not in a named source's actual published data, downgrade to non-numeric wording or say 'scope-dependent contractor estimate' without pretending it is industry research. BAD: 'NAHB data on drainage repair costs' — NAHB does not publish this. GOOD: 'UF IFAS termite guidance + FBC wood protection rule; cost depends on bid scope'.",
  "format_fit": true,
  ...
  "slide2_label": "1 short line connecting the stat to a homeowner's real decision. MAX 95 characters. Frame as a consequence, not a citation. Use only audit-grade sources. GOOD: 'Framing is 16.6% of new-home cost; material choice changes the bid (NAHB 2024).' BAD: 'Angi/HomeAdvisor aggregate repair data' (banned as primary proof) | 'According to NAHB, prices vary by material.' Always end with the source in parentheses.",
  "slide3_items": [
    {{"title": "Item 1 title", "sub": "1 line, MAX 115 characters, with a SPECIFIC NEW FACT that EXPANDS on hook_answer — name the physical consequence, code/use-case factor, or cost driver. NEVER restate hook_answer. Avoid unsupported dollar ranges. GOOD: 'UF IFAS flags subterranean/Formosan termites as major Florida wood risks.' BAD: 'Ignoring cracks causes damage.'"}},
    {{"title": "Item 2 title", "sub": "1 line, MAX 115 characters, with decision consequence — different angle from item 1"}},
    {{"title": "Item 3 title", "sub": "1 line, MAX 115 characters, with decision consequence — different angle from items 1 and 2"}}
  ],
  ...
  "slide4_body": "EXACTLY 2 sentences — no more. Hard limit: 260 characters total including spaces. This text appears beside an image on a fixed-size slide — if you write more than 260 characters the last sentence will be cut off on screen. First person, Mike speaking. Name the one specific thing they should ask or do first, and why it saves money. Conversational, not instructional. No promises, no superlatives. BANNED OPENERS: 'That\\'s the mistake I want you to avoid' | 'That\\'s why' | 'This is exactly why'. CLOSING CALLBACK REQUIRED: the LAST sentence must directly resolve the hook from slide 1. For COMPARISON topics: end with 'Go with [left material] when [specific condition]. Go with [right material] when [specific condition].' — non-negotiable. Count your characters before submitting.",
  "mentioned_people": [
    {{"name": "Full Name", "role_en": "role / why they're named", "slide": 4, "image_hint": "Wikipedia or editorial headshot search term"}}
  ],
  "sources": [
    "Source 1 — real publisher + exact report/code/guidance",
    "Source 2 — real publisher + exact report/code/guidance",
    "Source 3 — real publisher + exact report/code/guidance"
  ],
